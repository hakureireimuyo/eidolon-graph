...
脚本命名空间注入内核声明模型(`DataIn/DataOut/TriggerIn/ControlIn/ControlOut/StateField/ConfigField/InputGroup/Annot`)、信号电平(`ACTIVE/INACTIVE`)与触发策略常量(`ON_*`)。脚本只需定义一个名为 `Node` 的类:
...
class Node:
    ...
    control_out = [ControlOut("busy")]        # 信号节点:声明控制输出
    state = [StateField("calls", 0, Annot(int))]
    config = [ConfigField("offset", 0, Annot(int))]
    trigger_in = [TriggerIn("go")]            # 函数调用级触发入口
...
| — | 基类 final 方法 | 输入缓冲/信号自动传导/触发判定/状态提交不可重载(与代码节点相同) |

**可重载面与代码节点完全一致**:只重载各组处理逻辑与初始化逻辑;信号自动传导、输出投递、触发判定、组缓冲、状态提交是引擎基类 final。
